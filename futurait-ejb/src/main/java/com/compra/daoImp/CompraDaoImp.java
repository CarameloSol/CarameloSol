/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.compra.daoImp;

import com.compra.modelo.ComCompra;
import com.genericos.dao.GenericDaoImp;

import java.util.HashMap;
import java.util.List;
import javax.ejb.Stateless;
import javax.persistence.Query;
import com.compra.dao.ICompraDao;

/**
 *
 * @author Ricardo
 */
@Stateless
public class CompraDaoImp extends GenericDaoImp<ComCompra, Long>
        implements ICompraDao {

    public CompraDaoImp() {
        super(ComCompra.class);
    }

    @Override
    public List<ComCompra> buscar(ComCompra compra) {
        StringBuilder sql = new StringBuilder();
        HashMap<Object, Object> parametros = new HashMap();
        sql.append("Select t from ComCompra t where 1=1");
        if (compra.getId() != null) {
            sql.append(" and t.id=:id");
            parametros.put("id", compra.getId());
        }
        sql.append(" order by t.id  desc ");
        Query q = this.em.createQuery(sql.toString());
        parametros.keySet().forEach((key) -> {
            q.setParameter((String) key, parametros.get(key));
        });
        return q.getResultList();
    }

    @Override
    public List<ComCompra> busquedaPorFiltros(ComCompra compra) {
        StringBuilder sql = new StringBuilder();
        HashMap<Object, Object> parametros = new HashMap();
        sql.append("Select t from ComCompra t where 1=1");
        if (compra.getId() != null) {
            sql.append(" and t.id=:id");
            parametros.put("id", compra.getId());
        }
        
        sql.append(" order by t.id  desc ");
        Query q = this.em.createQuery(sql.toString());
        parametros.keySet().forEach((key) -> {
            q.setParameter((String) key, parametros.get(key));
        });
        return q.getResultList();
    }
    
    

    }
